...
export type PromptFileMode = 'file-reference' | 'inline-content';

export type BobPromptBuilderInput = {
...
export type BobPromptBuildResult = {
  prompt: string;
  mode: PromptFileMode;
...
export async function buildBobPrompt(input: BobPromptBuilderInput): Promise<BobPromptBuildResult> {
  validateInput(input);
...
function validateInput(input: BobPromptBuilderInput): void {
  if (!Number.isInteger(input.maxInlineBytes) || input.maxInlineBytes <= 0) {
    throw new BobPromptError('maxInlineBytes must be a positive integer', 'INVALID_CONFIG');
  }
}
...
async function readFileContent(workspacePath: string | undefined, file: SourceFile): Promise<string> {
