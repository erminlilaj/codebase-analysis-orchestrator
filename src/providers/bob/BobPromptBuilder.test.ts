...
beforeEach(async () => {
  workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'bob-prompt-'));
});
...
function makeInput(overrides: Partial<BobPromptBuilderInput> = {}): BobPromptBuilderInput {
  return {
    bundle: makeBundle(),
    question: {
      id: 'question-1',
...
describe('buildBobPrompt', () => {
  it('builds deterministic file-reference prompts with Bob @file references', async () => {
    const result = await buildBobPrompt(makeInput());
  ...
  it('builds inline-content prompts from workspace files', async () => {
    ...
    const result = await buildBobPrompt(makeInput({ mode: 'inline-content' }));
  ...
  it('rejects inline prompts that exceed maxInlineBytes', async () => {
    ...
    await expect(
      buildBobPrompt(makeInput({ mode: 'inline-content', maxInlineBytes: 9 })),
  ...
  it('uses a longer code fence when source contains backticks', async () => {
    ...
    const result = await buildBobPrompt(makeInput({ bundle, mode: 'inline-content' }));
  ...
  it('reports missing inline files as prompt errors', async () => {
    await expect(buildBobPrompt(makeInput({ mode: 'inline-content' }))).rejects.toBeInstanceOf(
      BobPromptError,
    );
    await expect(buildBobPrompt(makeInput({ mode: 'inline-content' }))).rejects.toMatchObject({
