...
as a child process, using `@relative/path` references against the workspace.
...
| 12: Bob Shell provider | In progress | CLI surface mapped (`bob ... --output-format stream-json`); adapter not yet implemented |
| 13: REST API | Complete | 7 route modules |
| 14: Exports (JSON/CSV/Markdown) | Complete | Streaming, paginated, backpressure-aware |
| 15: Tests (broaden coverage) | Partial | 123 unit tests passing; integration tests pending |
